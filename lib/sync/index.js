...
const { tidySvg, extractSvgFeatures } = require('./svg')
...
module.exports = async function sync ({ key, svg: svgDir, data: dataDir, json: jsonDir, diff, categories }) {
  if (!key) {
  ...
  const iconsMeta = Object.entries(file.components)
  ...
  const meta = {
    ...
    variants: (() => {
      return setConfig
        ? setConfig.variants
        : [...new Set(iconsMeta.reduce((arr, { variants }) => [...arr, ...Object.keys(variants)], []))]
    })(),
  ...
  meta.variants.forEach(variant => {
    createDir(`${svgDir}/${variant}`)
    createDir(`${jsonDir}/${variant}`)
  })
  ...
  const iconsMap = await readJSON(`${dataDir}/icons-map.json`).catch(() => ({}))
    ...
    return promise.then(async () => {
      ...
      await svgs.reduce((promise, { data: svgString }, i) => {
        return promise.then(async () => {
          ...
          const { node, svg } = await tidySvg(svgString, variant, setConfig)
          await saveSVG(name, svgDir, variant, svg)
          const json = await extractSvgFeatures(svg, variant)
          await saveJSON(`${jsonDir}/${variant}/${name}`, json)
          iconsMap[id] = svg
          downloaded++
  ...
  await saveJSON(`${dataDir}/icons-meta`, iconsMeta)
  await saveJSON(`${dataDir}/icons-map`, iconsMap)
